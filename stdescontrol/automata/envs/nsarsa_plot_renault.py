...
for k in range(cases):
    ...
    with open('automata/envs/testes/approve-A-90/case'+str(k+1)+'.csv', newline='') as csvfile:
        data = list(csv.reader(csvfile))
    reward = list(map(int, data[1]))
    probabilities = list(map(float, data[2]))
    
    start = time.time()
    env.reset(filename="automata/envs/SM/Renault2.xml", rewards=reward, stop_crit=1, last_action=last_action, products=10, probs=probabilities)
    end = time.time()
...
plt1 = intel.plot.bar(x=xlabel_name, stacked=True)
plt1.set_xlabel(xlabel_name)
plt1.set_ylabel("Number of Occurrences")

plt1 = randomic.plot.bar(x=xlabel_name, stacked=True)
plt1.set_xlabel(xlabel_name)
plt1.set_ylabel("Number of Occurrences")
